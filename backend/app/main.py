...
def get_class(number: int,course: str, db: Session = Depends(get_db)):
    result=conn.execute("""SELECT prof, COUNT(*), MAX(year),  SUM(atotal), SUM(btotal),SUM(ctotal),SUM(dtotal),SUM(ftotal),SUM(aftotal), SUM(qtotal), CAST(AVG(gpa) AS DECIMAL(3,2)) FROM gradestable\
